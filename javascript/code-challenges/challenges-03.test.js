...
const oddValues = (arr) => {
  // Solution code here...
  const newArr = arr.filter(number => number % 2 === 1);
  return newArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const newArr = arr.filter(item => forbiddenValues.indexOf(item) > -1 ? false : true);
  return newArr;
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  const newArr = arr.filter(item => item.baseStat > minBaseStat).map(item => item.stat.name);
  return newArr;
};
